<template>
                ...
                <div class="col-md-12">
                    <div class="row">
                        <div class="col-lg-3 col-md-6 col-sm-12">
                            <small class="label">Remote Contractor</small>
                            <template v-if="details.remoteContractor.success">
                                <v-select :options="details.remoteContractor.data.data" label="complete_name" v-model="details.selectedRemoteContractor" style="margin-top: 2px !important;" ></v-select>
                            </template>
                            <template v-else>
                        ...
                        <div class="col-lg-2 col-md-6  col-sm-12">
                            <small class="label">Date to Assigned</small>
                            <input type="date" class="form-control field" v-model="details.convertDate" style="font-size: 12pt; margin-top: 2px !important;"/>
                        </div>

                        <div class="col-lg-2 col-md-6 col-sm-12">
                            <MaterialButton @click="details.showConvertConfirmation = true" :disabled="!details.selectedRemoteContractor || !details.selectedClient" variant ="light" size="sm" style="margin-top: 25px !important;" class="btn-icon command-btn left-floater w-100">
                        ...
                        <div class="col-lg-2 col-md-12 col-sm-12">
                            <MaterialInput v-model="details.search" @keyup="getRecords" class="d-flex right-floater mx-1 border mb-2" icon="search mx-2" placeholder="Search User" size="sm" style="margin-top: 27px !important;" />
                        </div>
                    </div>
                </div>
            </div>
    ...
    <template>
        <teleport to="body">
            ...
            :isScrollable=true>
                <template #body>
                    <h6 class="text-center alert-message-modal">You are about to assign more Client to Remote Contractor.</h6>
                    <h6 class="text-center alert-message-modal">Do you want to continue?</h6>
                </template>
                
                <template #footer>
                    <material-button @click="assignMoreClientToRemoteContractor()" size="sm" variant="light" class="command-btn mb-0 me-2">
                        Yes
                    </material-button>
...
<script>
    ...
    export default {
        ...
        watch: {
            'details.selectedRemoteContractor'(newVal) {
                this.details.selectedClient = null;
                if(newVal) {
                    this.getClient();
                } else {
                    this.details.client.success = false;
                }
            }
        },
        methods: {
            ...
            getRecords() {
                ...
                axios({
                    method: 'get',
                    url: api.DEV_API_URL + `/admin/remote-contractor-registrant?with_clients=true&page=${self.details.pagination.current}&limit=${self.details.pagination.limit}&search=${self.details.search}`,
            ...
            getRemoteContractor() {
                ...
                axios({
                    method: 'get',
                    url: api.DEV_API_URL + '/admin/remote-contractor-registrant',
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
                })
                .then(function (response) {
                    self.details.remoteContractor = response.data;
                })
                .catch(function (errors) {
                    if(errors.response.status === 401) {
                        self.details.showAlertUnauthenticated.success = true;
            ...
            getClient() {
                ...
                var selectedRemoteContractorId = self.details.selectedRemoteContractor ? self.details.selectedRemoteContractor.id : null;
                axios({
                    method: 'get',
                    url: api.DEV_API_URL + `/admin/client-registrant/${selectedRemoteContractorId}`,
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
                })
                .then(function (response) {
            ...
            assignMoreClientToRemoteContractor() {
                ...
                axios({
                    method: 'post',
                    url: api.DEV_API_URL + '/admin/assign-more-client-to-remote-contractor',
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
                    data: {
                        remote_contractor_id: self.details.selectedRemoteContractor.id,
                        client_id: self.details.selectedClient.id,
                        convert_date: self.details.convertDate
                    }
                })
                .then(function (response) {
                    self.details.selectedRemoteContractor = null;
                    self.details.showConversionLoader = false;

                    self.details.alertBox.success = response.data.success;
                    self.details.alertBox.message = `<span style='color:#050312;'>Successfully assigned ${response.data.data.reg_firstname} ${response.data.data.reg_lastname}</span> to ${self.details.selectedClient.complete_name}.</span>`;
