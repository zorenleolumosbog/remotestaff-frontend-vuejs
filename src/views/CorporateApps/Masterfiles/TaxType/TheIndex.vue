        ...
        <div class="col-md-12">
            <div class="row mt-3 mb-n3">
                ...
                <div class="col-md-8 my-2 py-3">
                    ...
                    <MaterialInput v-model="details.search" @keyup="getRecords" class="d-flex right-floater mx-1 size border" icon="search mx-2" placeholder="Search Tax Type" size="sm"/>
                </div>
            </div>
            <div class="row">
                            ...
                            <table id="table" class="table table-hover table-striped table-bordered">
                                <thead class="bg-header">
                                    <tr>
                                        ...
                                        <th scope="col">Tax Type Code</th>
                                        <th scope="col">Tax Type Description</th>
                                    </tr>
                                </thead>
                                <tbody v-if="!details.showLoader">
                                    <template v-if="details.records.success">
                                        <tr :class="{'active-row': details.selectedRecords.includes(datum.id) }" v-for="datum in details.records.data.data" :key="datum.id" @click="setSeletedRecords(datum.id)">
                                            ...
                                            <td>{{ datum.short_desc }}</td>
                                            <td>{{ datum.long_desc }}</td>
                                        </tr>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                details: {
                    title: 'Manage Tax Type',
                    addTitle: 'Add Tax Type',
                    editTitle: 'Edit Tax Type',
                    showAdd: false,
                    showEdit: false,
        ...
        methods: {
            ...
            getRecords() {
                ...
                axios({
                    method: 'get',
                    url: api.DEV_API_URL + `/admin/tax-type?page=${self.details.pagination.current}&limit=${self.details.pagination.limit}&search=${self.details.search}`,
