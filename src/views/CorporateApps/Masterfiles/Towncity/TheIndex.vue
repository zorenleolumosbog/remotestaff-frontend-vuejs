        ...
        <div class="col-md-12">
            <div class="row mt-2 mb-n2">
                ...
                <div class="col-md-8 my-2 py-2">
                    ...
                    <MaterialInput v-model="details.search" @keyup="getRecords" class="d-flex right-floater mx-1 size border" icon="search mx-2" placeholder="Search Town/City" size="sm"/>
                </div>
            </div>
            <div class="row">
                                    ...
                                    <template v-if="details.records.success">
                                        <tr :class="{'active-row': details.selectedRecords.includes(datum.id) }" v-for="datum in details.records.data.data" :key="datum.id" @click="setSeletedRecords(datum.id)">
                                            ...
                                            <template v-if="datum.state.region">
                                                ...
                                                <td>{{ datum.state.region.description }}</td>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                details: {
                    title: 'Manage Town/City',
                    addTitle: 'Add Town/City',
                    editTitle: 'Edit Town/City',
                    showAdd: false,
                    showEdit: false,
        ...
        methods: {
            ...
            getRecords() {
                ...
                axios({
                    method: 'get',
                    url: api.DEV_API_URL + `/admin/towncity?page=${self.details.pagination.current}&limit=${self.details.pagination.limit}&search=${self.details.search}`,
