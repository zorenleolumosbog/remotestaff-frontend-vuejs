                                ...
                                <tbody v-if="!details.showLoader">
                                    <template v-if="details.records.success">
                                        <tr :class="{'active-row': details.selectedRecords.includes(datum.id) }" v-for="datum in details.records.data.data" :key="datum.id" @click="setSeletedRecords(datum.id)">
                                            ...
                                            <td>{{ datum.contract_status.description }}</td>
    ...
    export default {
        ...
        methods: {
            ...
            getRecords() {
                ...
                axios({
                    method: 'get',
                    url: api.DEV_API_URL + `/admin/cancellation-reason-type?page=${self.details.pagination.current}&limit=${self.details.pagination.limit}&search=${self.details.search}`,
