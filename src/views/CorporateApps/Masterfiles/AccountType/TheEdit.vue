        ...
        <teleport to="body">
            ...
            <template #body>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select v-model="details.link_acct_class_id" class="input-group-outline form-control p-2" id="classification">
                            <option selected value="">Choose Account Classification</option>
                            <option v-for="classification in accountClasssifications" :value="classification.id" :key="classification.id" >{{classification.description}}</option>
                        </select>
                        <template v-if="validations.errors.link_acct_class_id">
                            <span v-for="(error, index) in validations.errors.link_acct_class_id" :key="index" class="error-font">{{ error }}</span>
                        </template>
                    </div>
                </template>
                <template #footer>
                    <material-button @click="update" variant="contained" color="success" size="sm" class="mb-0">
                        Update
                    </material-button>
                </template>
            </standard-modal>
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            axios({
                method: 'get',
                url: api.DEV_API_URL + `/admin/account-classification`,
                headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
            })
            .then(function (response) {
                if (response.data.success) {
                    self.accountClasssifications = response.data.data.data

                    console.log(self.accountClasssifications)
                }
            })
        ...
        methods: {
            ...
            getRecord() {
                ...
                axios({
                    method: 'get',
                    url: api.DEV_API_URL + '/admin/account-type/' + self.selectedId,
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
                })
                .then(function (response) {
                    self.details.description = response.data.data.description;
                    self.details.link_acct_class_id = response.data.data.link_acct_class_id;
            ...
            update() {
                ...
                axios({
                    method: 'put',
                    url: api.DEV_API_URL + `/admin/update-account-type/${self.selectedId}`,
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
                    data: {
                        description: self.details.description,
                        link_acct_class_id: self.details.link_acct_class_id
                    }
                })
                .then(function (response) {
                    self.$emit("refresh");
