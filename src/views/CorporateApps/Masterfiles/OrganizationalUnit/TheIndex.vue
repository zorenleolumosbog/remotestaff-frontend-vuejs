<template>
    <div class="container-fluid mx-1 div-border">
        <div class="col-md-12">
            <div class="row mt-3 mb-3">
                ...
                <div class="col-md-8 my-2">
                    ...
                    <MaterialInput v-model="details.search" @keyup="getRecords" class="d-flex right-floater mx-1 size border" icon="search mx-2" placeholder="Search Organizational Unit" size="sm"/> 
                </div>
            </div>
            <div class="row">
                            ...
                            <table id="table" class="table table-bordered table-hover table-striped">
                                <thead class="bg-header">
                                    <tr>
                                        ...
                                        <th scope="col">OU Name</th>
                                        <th scope="col">Registered Legal Name</th>
                                ...
                                <tbody v-if="!details.showLoader">
                                    <template v-if="details.records.success">
                                        <tr :class="{'active-row': details.selectedRecords.includes(datum.id) }" v-for="datum in details.records.data.data" :key="datum.id" @click="setSeletedRecords(datum.id)">
                                            ...
                                            <td>{{ datum.ou_name }}</td>
                                            <td>{{ datum.registered_legal_name }}</td>
                                            <td>
                                                <div class="form-check form-switch d-inline-block">
                                                    <input :checked="datum.is_hq" type="checkbox" class="form-check-input" onclick="return false;">
                                                </div>
                                            </td>
                                        </tr>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                details: {
                    title: 'Manage Organizational Unit',
                    addTitle: 'Add Organizational Unit',
        ...
        methods: {
            ...
            getRecords() {
                ...
                axios({
                    method: 'get',
                    url: api.DEV_API_URL + `/admin/organizational-unit?page=${self.details.pagination.current}&limit=${self.details.pagination.limit}&search=${self.details.search}`,
