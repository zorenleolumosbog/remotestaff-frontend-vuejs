            ...
            <template #body>
                    <div class="row">
                        <div class="form-group contract_status mb-3">
                            <label for="filetype_id" class="form-label">Contract Note</label>
                            <v-select class="input-group-outline reach-inside-border" :clearable="false" :options="details.fileTypes" label="description" v-model="details.fileTypeId" style="margin-top: 2px !important;" ></v-select>
    ...
    export default {
        ...
        methods: {
            ...
            getRecord() {
                ...
                axios({
                    method: 'get',
                    url: api.DEV_API_URL + `/admin/contract-note/${self.selectedId}`,
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
                })
                .then(function (response) {
                    self.details.fileTypeId = {
                        id: response.data.data.link_filetype_id,
                        description: response.data.data.filetype.description
                    }
                    self.details.notes = response.data.data.notes;
            ...
            update() {
                ...
                axios({
                    method: 'post',
                    url: api.DEV_API_URL + '/admin/update-contract-note/' + self.selectedId,
                    headers: { 
