        ...
        <div class="col-md-12">
            <div class="row mt-3 mb-3">
                ...
                <div class="col-md-8 my-2">
                    ...
                    <MaterialInput v-model="details.search" @keyup="getRecords" class="d-flex right-floater mx-1 size border" icon="search mx-2" placeholder="Search Section" size="sm"/>
                </div>
            </div>
            <div class="row">
                            ...
                            <table id="table" class="table table-hover table-striped table-bordered">
                                <thead class="bg-header">
                                    <tr>
                                        ...
                                        <th scope="col" data-sort-method='none' class="action"></th>
                                    </tr>
                                </thead>
                                <tbody v-if="!details.showLoader">
                                    <template v-if="details.records.success">
                                        <tr :class="{'active-row': details.selectedRecords.includes(datum.id) }" v-for="datum in details.records.data.data" :key="datum.id" @click="setSeletedRecords(datum.id)">
                                            <td data-sort-method="number" class="ps-3">{{ datum.id }}</td>
                                            <td>{{ datum.filetype_description }}</td>
                                            <td>{{ datum.notes }}</td>
                                            <td data-sort-method='none'><a v-if="datum.img_url" :href="datum.img_url" target="_blank" class="btn btn-sm btn-icon command-btn mb-0">Attachment</a></td>
                                        </tr>
                                        <tr height="40" v-for="tr in 10 - parseInt(details.records.data.data.length)" :key="tr">
                                            <td></td>
                                            <td></td>
                                            <td></td>
                                            <td></td>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                details: {
                    addTitle: 'Add Contract Note',
                    editTitle: 'Edit Contract Note',
                    showAdd: false,
                    showEdit: false,
        ...
        methods: {
            ...
            getRecords() {
                ...
                axios({
                    method: 'get',
                    url: api.DEV_API_URL + `/admin/contract-note?page=${self.details.pagination.current}&limit=${self.details.pagination.limit}&search=${self.details.search}`,
