        ...
        <div class="col-md-12">
            <div class="row mt-2 mb-n2">
                ...
                <div class="col-md-8 my-2 py-2">
                    ...
                    <MaterialInput v-model="details.search" @keyup="getRecords" class="d-flex right-floater mx-1 size border" icon="search mx-2" placeholder="Search Baranggay/Village" size="sm"/>
                </div>
            </div>
            <div class="row">
                                ...
                                <tbody v-if="!details.showLoader">
                                    <template v-if="details.records.success">
                                        <tr :class="{'active-row': details.selectedRecords.includes(datum.id) }" v-for="datum in details.records.data.data" :key="datum.id" @click="setSeletedRecords(datum.id)">
                                            ...
                                            <td>{{ datum.towncity.state.region.country.long_desc }}</td>
                                            <td>{{ datum.towncity.state.region.description }}</td>
                                            <td>{{ datum.towncity.state.description }}</td>
                                            <td>{{ datum.towncity.description }}</td>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                details: {
                    title: 'Manage Barangay/Village',
                    addTitle: 'Add Barangay/Village',
                    editTitle: 'Edit Barangay/Village',
                    showAdd: false,
                    showEdit: false,
        ...
        methods: {
            ...
            getRecords() {
                ...
                axios({
                    method: 'get',
                    url: api.DEV_API_URL + `/admin/barangay?page=${self.details.pagination.current}&limit=${self.details.pagination.limit}&search=${self.details.search}`,
