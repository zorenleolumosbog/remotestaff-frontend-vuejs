                ...
                <template #body>
                    <div class="row">
                        <div class="form-group mb-3">
                            <label for="registrant" class="form-label">Registrant</label>
                            <select v-model="details.registrantId" class="input-group-outline form-control p-2" id="registrant">
                                <option selected value="">Choose Registrant</option>
                                <template v-if="registrant">
                                    <template v-for="registrant in registrant.data" :key="registrant.id">
                            ...
                            <template v-if="validations.errors.registrant_id">
                                <span v-for="(error, index) in validations.errors.registrant_id" :key="index" class="error-font">{{ error }}</span>
                            </template>
                        </div>
                    </div>
                    <div class="form-group mb-3">
...
<script>
    ...
    export default {
        ...
        props: ['title', 'showEdit', 'selectedId', 'registrant'],
        methods: {
            submitForm() {
            ...
            getRecord() {
                ...
                axios({
                    method: 'get',
                    url: api.DEV_API_URL + `/admin/onboard-registration-expiry/${self.selectedId}`,
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
                })
                .then(function (response) {
                    self.details.registrantId = response.data.data.link_preregid;
            ...
            update() {
                ...
                axios({
                    method: 'put',
                    url: api.DEV_API_URL + '/admin/update-onboard-registration-expiry/' + self.selectedId,
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
                    data: {
                        registrant_id: self.details.registrantId,
