            ...
            :isScrollable=true>
                <template #body>
                    <div class="form-group mb-3">
                        <label for="short_desc" class="form-label">Country Code</label>
                        <input v-model="details.shortDescription" type="text" class="input-group-outline form-control p-2" id="short_desc" placeholder="">
                        <template v-if="validations.errors.short_desc">
                            <span v-for="(error, index) in validations.errors.short_desc" :key="index" class="error-font">{{ error }}</span>
                        </template>
                    </div>
                    <div class="form-group mb-3">
                        <label for="long_desc" class="form-label">Country Name</label>
                        <input v-model="details.longDescription" type="text" class="input-group-outline form-control p-2" id="long_desc" placeholder="">
                        <template v-if="validations.errors.long_desc">
                            <span v-for="(error, index) in validations.errors.long_desc" :key="index" class="error-font">{{ error }}</span>
                        </template>
                    </div>
                    <div class="form-group mb-3">
                        <label for="is_hq" class="form-label">With Region</label>
                    </div>
                    <div class="col">
                        <div class="d-inline-block me-3">No</div>
                        <div class="form-check form-switch d-inline-block" >
                            <input type="checkbox" v-model="details.withRegion" :value="1" class="form-check-input" id="is_hq">
    ...
    export default {
        ...
        methods: {
            ...
            getRecord() {
                ...
                axios({
                    method: 'get',
                    url: api.DEV_API_URL + '/admin/country/' + self.selectedId,
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
                })
                .then(function (response) {
                    self.details.shortDescription = response.data.data.short_desc;
                    self.details.longDescription = response.data.data.long_desc;
                    self.details.withRegion = response.data.data.with_region ? true : false;
            ...
            update() {
                ...
                axios({
                    method: 'put',
                    url: api.DEV_API_URL + `/admin/update-country/${self.selectedId}`,
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
                    data: {
                        short_desc: self.details.shortDescription,
                        long_desc: self.details.longDescription,
                        with_region: self.details.withRegion
                    }
                })
                .then(function (response) {
                    self.$emit("refresh");
