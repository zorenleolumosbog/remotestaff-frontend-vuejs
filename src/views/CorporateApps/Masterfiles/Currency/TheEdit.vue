        ...
        <teleport to="body">
            ...
            <template #body>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input v-model="details.code" type="text" class="input-group-outline form-control p-2" id="code" placeholder="">
                        <template v-if="validations.errors.code">
                            <span v-for="(error, index) in validations.errors.code" :key="index" class="error-font">{{ error }}</span>
                        </template>
                    </div>
                    <div class="form-group mb-3">
                        <label for="symbol" class="form-label">Symbol</label>
                        <input v-model="details.symbol" type="text" class="input-group-outline form-control p-2" id="symbol" placeholder="">
                        <template v-if="validations.errors.symbol">
                            <span v-for="(error, index) in validations.errors.symbol" :key="index" class="error-font">{{ error }}</span>
                        </template>
                    </div>
                    <div class="form-group mb-3">
    ...
    export default {
        ...
        methods: {
            ...
            getRecord() {
                ...
                axios({
                    method: 'get',
                    url: api.DEV_API_URL + `/admin/currency/${self.selectedId}`,
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
                })
                .then(function (response) {
                    self.details.countryId = response.data.data.link_country_id;
                    self.details.code = response.data.data.code;
                    self.details.symbol = response.data.data.symbol;
            ...
            update() {
                ...
                axios({
                    method: 'put',
                    url: api.DEV_API_URL + '/admin/update-currency/' + self.selectedId,
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
                    data: {
                        country_id: self.details.countryId,
                        code: self.details.code,
                        symbol: self.details.symbol,
