<template>
    <div class="container-fluid"> <!--removed div-bordered-->
        <div class="col-md-12">
            <div class="row mt-3 mb-n3">
                ...
                <div class="col-md-8 my-2 py-3">
                    ...
                    <MaterialInput v-model="details.search" @keyup="getRecords" class="d-flex right-floater mx-1 size border" icon="search mx-2" placeholder="Search Tax Rate" size="sm"/>
                </div>
            </div>
            <div class="row">
                            ...
                            <table id="table" class="table table-hover table-striped table-bordered">
                                <thead class="bg-header">
                                    <tr>
                                        ...
                                        <th scope="col">Tax Rate Description</th>
                                        <th scope="col">Rate</th>
                                        <th scope="col">State Applied</th>
                                    </tr>
                                </thead>
                                <tbody v-if="!details.showLoader">
                                    <template v-if="details.records.success">
                                        <tr :class="{'active-row': details.selectedRecords.includes(datum.id) }" v-for="datum in details.records.data.data" :key="datum.id" @click="setSeletedRecords(datum.id)">
                                            ...
                                            <template v-if="datum.state_applied">
                                                <template v-if="datum.state.region">
                                                    <td>{{ datum.state.region.country.long_desc }}</td>
                                                    <td>{{ datum.state.description }}</td>
                                                </template>
                                                <template v-else>
                                                    <td>{{ datum.state.country.long_desc }}</td>
                                                    <td>{{ datum.state.description }}</td>
                                                </template>
                                            </template>
                                            <template v-else>
                                                <td>{{ datum.country.long_desc }}</td>
                                                <td>{{  }}</td>
                                            </template>
                                            <td>{{ datum.tax_type.long_desc }}</td>
                                            <td>{{ datum.description }}</td>
                                            <td>{{ datum.rate }}</td>
                                            <td>
                                                <div class="form-check form-switch d-inline-block">
                                                    <input :checked="datum.state_applied" type="checkbox" class="form-check-input" onclick="return false;">
                                                </div>
                                            </td>
                                        </tr>
    ...
    export default {
        ...
        methods: {
            ...
            getRecords() {
                ...
                axios({
                    method: 'get',
                    url: api.DEV_API_URL + `/admin/tax-rate?page=${self.details.pagination.current}&limit=${self.details.pagination.limit}&search=${self.details.search}`,
