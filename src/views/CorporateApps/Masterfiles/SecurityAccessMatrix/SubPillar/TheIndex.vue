...
const headers = ref(["ID", "Description", "Date Created", "Pillar"]);
const gridColumns = ref(["id", "description", "datecreated", "pillar_name"]);
const data = ref([]);
...
const handleAddRecord = async (data: any) => {
  ...
  const response = await store.addSubPillar(data);
...
const handleUpdateRecord = async (data: any) => {
  ...
  modalLoaderShow.value = true;
  isLoading.value = true;
  const response = await store.updateSubPillar(data.id, {
    link_pillar_id: data.link_pillar_id,
...
const deleteRecord = async () => {
  ...
  console.log(item);
  const response = await store.deleteSubPillar(item.id);
  if (response?.data.success) {
    isLoading.value = false;
    selectedRows.value = [];
    selectedIndex.value = "";
