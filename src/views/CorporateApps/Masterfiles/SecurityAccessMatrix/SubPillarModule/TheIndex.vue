...
const assignFields = () => {
  if (store.subpillar_modules) {
    data.value = store.subpillar_modules;
...
const handleAddRecord = async (data: any) => {
  ...
  const response = await store.addSubPillarModule(data);
...
const handleUpdateRecord = async (data: any) => {
  ...
  modalLoaderShow.value = true;
  isLoading.value = true;
  const response = await store.updateSubPillarModule(data.id, {
    link_sub_pillar_id: data.link_sub_pillar_id,
...
const deleteRecord = async () => {
  ...
  console.log(item);
  const response = await store.deleteSubPillarModule(item.id);
  if (response?.data.success) {
    isLoading.value = false;
    selectedRows.value = [];
    selectedIndex.value = "";
    ...
    <div class="row">
      ...
      <div class="col-12">
        ...
        <div style="float: right">
          <DataPagination
            @goToPage="goToPage"
            :records="store.subpillarModuleLinks"
          />
        </div>
      </div>
      <div class="col-12">
        <Teleport to="body">
          <TheAdd
            v-if="showAddModal"
            :show="showAddModal"
            :options="store.subPillarOptions"
