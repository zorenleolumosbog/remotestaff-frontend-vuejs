...
const assignFields = () => {
  if (store.menu_headings) {
    data.value = store.menu_headings;
...
const handleAddRecord = async (data: any) => {
  ...
  const response = await store.addMenuHeading(data);
...
const handleUpdateRecord = async (data: any) => {
  ...
  modalLoaderShow.value = true;
  isLoading.value = true;
  const response = await store.updateMenuHeading(data.id, {
    link_sub_pillar_module_id: data.link_sub_pillar_module_id,
...
const deleteRecord = async () => {
  ...
  console.log(item);
  const response = await store.deleteMenuHeading(item.id);
  if (response?.data.success) {
    isLoading.value = false;
    selectedRows.value = [];
    selectedIndex.value = "";
    ...
    <div class="row">
      ...
      <div class="col-12">
        ...
        <div style="float: right">
          <DataPagination
            @goToPage="goToPage"
            :records="store.menuheadingsLinks"
          />
        </div>
      </div>
      <div class="col-12">
