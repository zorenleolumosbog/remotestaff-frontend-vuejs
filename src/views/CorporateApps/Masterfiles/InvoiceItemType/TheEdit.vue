        ...
        <teleport to="body">
            ...
            <template #body>
                    <div class="form-group mb-3">
                        <label for="description" class="form-label">Invoice Item Type</label>
                    ...
                    <div class="col">
                        ...
                        <div class="form-check form-switch d-inline-block" >
                            <input type="checkbox" v-model="details.isPercentage" :value="1" class="form-check-input" id="is_percentage">
                            <label class="form-check-label">Yes</label>
                        </div>
                    </div>
                    <div v-if="details.isPercentage" class="form-group mb-3">
                        <label for="percentage" class="form-label">Percentage</label>
                        <input v-model="details.percentage" type="text" class="input-group-outline form-control p-2" id="percentage" placeholder="Percentage">
                        <template v-if="validations.errors.percentage">
                            <span v-for="(error, index) in validations.errors.percentage" :key="index" class="error-font">{{ error }}</span>
                        </template>
                    </div>
                </template>
                <template #footer>
    ...
    export default {
        ...
        methods: {
            ...
            getRecord() {
                ...
                axios({
                    method: 'get',
                    url: api.DEV_API_URL + '/admin/invoice-item-type/' + self.selectedId,
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
                })
                .then(function (response) {
                    self.details.description = response.data.data.description;
                    self.details.isPercentage = response.data.data.is_percentage ? true : false;
                    self.details.percentage = response.data.data.percentage;
            ...
            update() {
                ...
                axios({
                    method: 'put',
                    url: api.DEV_API_URL + `/admin/update-invoice-item-type/${self.selectedId}`,
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
                    data: {
                        description: self.details.description,
                        is_percentage: self.details.isPercentage ? self.details.isPercentage : null,
                        percentage: self.details.isPercentage ? self.details.percentage : null
                    }
                })
                .then(function (response) {
                    self.$emit("refresh");
