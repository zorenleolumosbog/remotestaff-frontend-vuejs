        ...
        <div class="col-md-12">
            <div class="row mt-3 mb-3">
                ...
                <div class="col-md-8 my-2">
                    ...
                    <MaterialInput v-model="details.search" @keyup="getRecords" class="d-flex right-floater mx-1 size border" icon="search mx-2" placeholder="Search Invoice Item Type" size="sm"/>
                </div>
            </div>
            <div class="row">
                            ...
                            <table id="table" class="table table-hover table-striped table-bordered">
                                <thead class="bg-header">
                                    <tr>
                                        ...
                                        <th scope="col">Invoice Item Type</th>
                                    </tr>
                                </thead>
                                <tbody v-if="!details.showLoader">
                                    <template v-if="details.records.success">
                                        <tr :class="{'active-row': details.selectedRecords.includes(datum.id) }" v-for="datum in details.records.data.data" :key="datum.id" @click="setSeletedRecords(datum.id)">
                                            ...
                                            <td>{{ datum.description }} <span v-if="datum.is_percentage">({{ datum.percentage }}%)</span></td>
                                        </tr>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                details: {
                    title: 'Manage Invoice Item Type',
                    addTitle: 'Add Invoice Item Type',
                    editTitle: 'Edit Invoice Item Type',
                    showAdd: false,
                    showEdit: false,
        ...
        methods: {
            ...
            getRecords() {
                ...
                axios({
                    method: 'get',
                    url: api.DEV_API_URL + `/admin/invoice-item-type?page=${self.details.pagination.current}&limit=${self.details.pagination.limit}&search=${self.details.search}`,
