    ...
    <!-- <StandardLayout>
        ...
        <template #main> -->
            <div class="container-fluid py-1">
                <div class="col-md-12">
                    <div class="row">
                        <div class = "col-md-12 row filters" style = "padding-right: 0">
                            <div class="col-sm-5 row">
                                <div class ="col-md-4 date-label">Select Client</div>
                                <div class ="col-md-8">
                            ...
                            <div class="col-sm-5 row mb-2">
                                <div class ="col-md-4 date-label">Select Invoice</div>
                                <div class ="col-md-8">
                    ...
                    <div class="row mt-5 mb-2">
                        <div class="col-xl-12">
                            <!-- <MaterialButton @click="details.showDelete = !details.showDelete" 
                            ...
                            <MaterialButton :disabled="!details.invoiceDetails.success" @click="details.showAdd = !details.showAdd" size="md" class="btn-icon mx-1 yes-btn float-end">
                                <div class="buttons d-flex align-items-center">
                                    Add Item
                                </div>
                            </MaterialButton>
                                        ...
                                        <tbody v-if="!details.showLoader">
                                            <template v-if="details.invoiceDetails.success">
                                                <tr :class="{'active-row': details.selectedRecords.includes(datum.id) }" v-for="datum in details.invoiceDetails.data.data" :key="datum.id" @click="setSeletedRecords(datum)">
                                                    ...
                                                    <td>{{ datum.link_invoice_hdr_id }}</td>
                                                    <td>{{ datum.particular }}</td>
                                                    <td v-if="datum.invoice_item_type">{{ datum.invoice_item_type.description }} <span v-if="datum.invoice_item_type.is_percentage">({{ datum.invoice_item_type.percentage }}%)</span></td>
                                                    <td v-else></td>
                                                    <td>{{ datum.hourly_rate }}</td>
                                                    <td>{{ datum.total_hours }}</td>
                                                    <td>{{ datum.amount_add_on }}</td>
                                                </tr>
            ...
            v-if="details.showAdd" 
                :selectedInvoiceId="details.selectedInvoice.id"
    ...
    export default {
        ...
        data() {
            return {
                details: {
                    ...
                    editTitle: 'Edit item to Invoice',
                    showAdd: false,
                    showEdit: false,
        ...
        methods: {
            ...
            getClients() {
                ...
                axios({
                    method: 'get',
                    url: api.DEV_API_URL + '/accounting/client-invoice-header',
            ...
            getClientInvoiceDetails() {
                ...
                axios({
                    method: 'get',
                    url: api.DEV_API_URL + `/accounting/client-invoice-header-detail/${self.details.selectedInvoice.id}?page=${self.details.pagination.current}&limit=${self.details.pagination.limit}&search=${self.details.search}`,
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
                })
                .then(function (response) {
            ...
            setSeletedRecords(datum) {
                ...
                if(this.details.selectedRecords.length === 1 && datum.amount_add_on) {
