<template>
    <template v-if="!details.showLoader && !details.alertBox.success">
        <teleport to="body">
            ...
            :isScrollable=true>
                <template #body>
                    <h6 class="text-center">You have selected {{ selectedIds.length === 1 ? 'this record' : 'these records' }} to be deleted.</h6>
        ...
        methods: {
            ...
            remove() {
                ...
                this.selectedIds.forEach((selectedId) => {
                    axios({
                        method: 'delete',
                        url: api.DEV_API_URL + `/accounting/delete-client-invoice-header-item-detail/${selectedId}`,
