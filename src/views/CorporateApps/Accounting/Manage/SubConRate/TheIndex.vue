    ...
    export default {
        ...
        methods : {
            ...
            getSubConRate ( page ) {
                ...
                axios.get(this.API_URL+'/accounting/client-subcon-rate',{
                    params:{
                        limit: this.rowLimit,
                    ...
                    if ( response.data.success ) {
                        const subConRate = response.data.data.data;

                        this.currentPage = response.data.data.current_page;
                        this.contractsData = response.data;

                        for ( const x in subConRate ) {
                            subConRate[x]['selected'] = false;
                            subConRate[x]['effective_date_to'] = this.formatDate(subConRate[x]['effective_date_to'])
                            subConRate[x]['effective_date_from'] = this.formatDate(subConRate[x]['effective_date_from'])
                        }

                        this.subConRate = subConRate;
            ...
            addSubConRate ( data ) {
                ...
                axios.post(this.API_URL+'/accounting/store-client-subcon-rate',
                    data
                ,{
            ...
            editSubConRate ( data ) {
                ...
                axios.post(this.API_URL+'/accounting/update-client-subcon-rate',
                    data
                ,{
                    headers: { Authorization: `Bearer ${this.TOKEN}` }
                }).then((response)=>{
                    ...
                    if (response.data.success) {
                        ...
                        this.subConRate[this.selectedIndex] = response.data.data
                        this.subConRate[this.selectedIndex]['selected'] = true;
                        this.isLoading = false;
            ...
            deleteSubConRate () {
                ...
                axios.post(this.API_URL+'/accounting/delete-client-subcon-rate',
                ...
                ,{
                    ...
                    let leftData = [];
                    for ( const x in this.subConRate ) {
                        if ( this.selectedRows.includes(this.subConRate[x]['id'])) {
                            continue;
                        }
                        leftData.push(this.subConRate[x]);
                    }

                    this.subConRate = leftData;
                    this.selectedRows = [];
                    this.selectedIndex = "";
                    this.isBtnDisableDelete = true;
                    this.isLoading = false;
            ...
            selectRow ( selected ) {
                ...
                for(const x in this.subConRate){
                    ...
                    if (selected.id == this.subConRate[x]['id']) {
                        index = x;

                        if (  this.selectedRows.includes(this.subConRate[x]['id']) ) {
                            this.selectedRows = this.selectedRows.filter(item => item !== this.subConRate[x]['id']);
                        }else {
                            this.selectedRows.push(this.subConRate[x]['id']);
                        }
                    }
                ...
                if ( this.selectedRows.length == 1 ) {
                    this.selectedIndex = this.subConRate.findIndex((value) => value.id === this.selectedRows[0]);
                }

                this.subConRate[index].selected = this.subConRate[index].selected ? false : true;
        ...
        computed : {
            ...
            getSelectedRow (){
                ...
                return this.subConRate[i] ? this.subConRate[i] : "";
                ...
                <div class="row">
                    <div class="col-12 mb-2">
                        ...
                        <div class="search-bar mx-4 mt-2 mt-md-0">
                            ...
                            v-model="searchQuery" @keyup.enter="getSubConRate(1)"
                ...
                <Teleport to="body">
                    ...
                    <StandardModal 
                        ...
                        <template #footer>
                            ...
                            <div v-else>
                                <MaterialButton size="sm" variant="contained" color="success" class="command-btn mb-0" @click="toggleConfirmModal('yes')">
                                    Yes
                                </MaterialButton>&nbsp;
...
.multiselect-subcon-rate .multiselect-option.is-selected.is-pointed {
    background: #c4bcfc;
    color: #3f3f3f;
}
.multiselect-subcon-rate .multiselect-option.is-selected {
    background: #c4bcfc;
    color: #3f3f3f;
}
.multiselect-subcon-rate .multiselect-dropdown {
