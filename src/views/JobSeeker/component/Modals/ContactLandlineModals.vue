...
<script>
    ...
    export default {
        name: "ContactLandlineModals",
        props: ['show', 'selectedLandline', 'type', 'selectedIndex', 'isPrimaryLandline', 'hasPrimary', 'validateLandline'],
        ...
        data() {
            return {
                ...
                countryCode: {'country': 'PH', 'code': ''},
                landline: { 
                    'number': '',  
                    'primary': this.isPrimaryLandline,
                    'isNewPrimary': false
                },
                landlineResult: "",
                isDisableBtn: true,
        ...
        methods: {
            handleDelete(action){
                if(action == 'yes'){
                    this.$emit('deleteLandline', this.selectedIndex);
            ...
            validate(action){
                ...
                this.landline.isNewPrimary = (this.hasPrimary == true 
                && ( this.landline.primary != this.isPrimaryLandline )) 
                    ? true : false;

                if ( !this.landline.primary ) {
                    this.landline.isNewPrimary = false;
                }

                if(action == 'add'){
                    this.$emit('addLandline', this.landline);
                }else{
                    this.landline['index'] = this.selectedIndex;
                    this.$emit('updateLandline', this.landline);
                }

                this.is_valid_landline = false;
            ...
            setPrimary () {
                this.landline.primary = !this.landline.primary;
            ...
            setDefaultValue () {  
                ...
                if ( this.selectedLandline ){
                    const selectedLandline = this.selectedLandline
                    const parsedNumber = parsePhoneNumber(selectedLandline)
                    this.countryCode.country = parsedNumber.country;
                    this.landline.number = parsedNumber.nationalNumber;

                    this.validateLandlineNumber( this.selectedLandline )
                }
            }
    ...
    <div v-if="type=='add'">
        ...
        <StandardModal
            ...
            <template #body>
                ...
                <div class="row mb-2 mx-0" v-if="validateLandline.isAdded == true">
                    <MaterialAlert color="danger">{{ validateLandline.message }}</MaterialAlert>
                </div>
                ...
                <div class="row my-3">
                    ...
                    <div class="form-group col-8 col-md-8 number">
                        <label for="landline_number" class="labels my-0">Landline number</label>
                        <input type="number" class="input-group-outline form-control  p-2" 
                            id="landline_number"
                            v-model="landline.number"
                            :class="isValid"
                            @keyup="hanndleChange"
                            autocomplete="off">
                    </div>
            ...
            <template #footer>
                ...
                <MaterialButton variant="contained" color="white" size="sm" class="command-btn mb-0" @click="validate('add')"
    ...
    <div v-if="type=='confirm'">
        ...
        <StandardModal 
            ...
            <template #footer>
                ...
                <MaterialButton size="sm" variant="contained" color="white" class="yes-btn mb-0" @click="$emit('continueEdit', 'landline')">
                    Yes
                </MaterialButton>&nbsp;
