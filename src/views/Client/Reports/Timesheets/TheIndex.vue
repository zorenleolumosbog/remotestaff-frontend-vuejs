                                ...
                                <tbody v-if="!details.showLoader">
                                    <template v-if="details.records.success">
                                        <tr v-for="(datum, index) in details.records.data.data" :key="index">
                                            ...
                                            <td class="text-end">{{ getActualHoursWork(datum.work_total_hours, datum.lunch ? datum.lunch.work_total_hours : 0) }}</td>
        ...
        methods: {
            ...
            getTimeSheet() {
                ...
                if(this.searchParams.dataSource === 'current'){
                    axios({
                        method: 'get',
                        url: api.DEV_API_URL + `/client/remote-contractor-attendance?page=${self.details.pagination.current}&limit=${self.details.pagination.limit}&start_date=${self.searchParams.startDate}&end_date=${self.searchParams.endDate}&client_id=${self.client_value}&remote_contractor_id=${self.searchParams.staff_value}`,
